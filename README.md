...
- 微信公众号：当前已有完整 adapter，支持普通图文和图片消息（小绿书）的本地检查、预览、Wenyan 输入生成，并在确认后写入公众号草稿箱。
...
先准备个人数据工作区。推荐把任意个人内容 Git 仓库克隆到 ArcSocial 根目录下的 `workspace/`。`workspace/` 是个人数据，不属于 ArcSocial 开源仓库，默认会被主仓库忽略：
...
- `workspace/` 是推荐的个人数据挂载位置，会被 ArcSocial 主仓库忽略；它不属于 ArcSocial 开源项目本体。

用户自己的文章、草稿、平台适配稿、发布素材、指标和私有平台数据，应该放在任意独立 Git 仓库中，并以未被 ArcSocial 主仓库跟踪的本地工作区形式挂载到 `workspace/`。不要把 `.gitmodules`、`workspace` submodule 或个人内容仓库绑定提交到 ArcSocial 开源仓库。这个数据仓库可以使用 `templates/workspace/` 初始化，也可以是已有仓库，只要目录结构满足 ArcSocial 的读写约定即可。
...
workspace/             推荐的个人数据挂载位置，被主仓库忽略，不提交到开源项目。
```
