...
5. 在写作、标题和表达校准时读取用户偏好，但不把个人偏好写成通用规则。
...
<workspacePath>/preferences/       用户长期写作、标题、平台和视觉偏好。
```
...
创建或重写草稿前，如果存在 `<workspacePath>/preferences/writing-style.md`、`<workspacePath>/preferences/title-style.md` 或 `<workspacePath>/preferences/reusable-blocks.md`，先读取并用于控制表达口吻、标题力度和固定区块。偏好文件不存在时不阻塞；当前用户明确指令优先于历史偏好。
...
平台适配前，如果存在对应平台偏好文件，例如 `<workspacePath>/preferences/wechat-style.md`，先读取并应用到段落节奏、标题取向、摘要口径和可复用区块。偏好只能影响表达取向，不能覆盖事实边界、平台规则或上层发布门禁。
...
- 保证用户原始输入可恢复。
- 区分记录、标准草稿和平台适配。
- 区分通用写作方法和用户个人偏好；个人语气、审美、标题力度和固定结尾只来自当前指令或 `<workspacePath>/preferences/`。
- 不编造个人经历、数据、引用或外部事实。
- 不确定的判断写入开放问题。
...
- `preferences_used`: 本轮读取的偏好文件；没有则写 none。
- `next_step`: 建议的下一步。
